...
struct EnvironmentConfig {
    ...
    private init() {
        self.env = EnvConfig.current
        self.config = Self.loadConfig(for: env)
    }

    // MARK: - Load Configuration

    private static func loadConfig(for environment: EnvConfig) -> [String: String] {
    ...
    private static func loadFromEnvFile(environment: EnvConfig) -> [String: String]? {
    ...
    private static func loadFromInfoPlist() -> [String: String]? {
        guard let bundle = Bundle.main.path(forResource: "Config", ofType: "plist"),
            let configDict = NSDictionary(contentsOfFile: bundle) as? [String: Any]
        else {
            return nil
        }
    ...
    private static func defaultConfig(for environment: EnvConfig) -> [String: String] {
        switch environment {
        case .development:
            return [
...
extension EnvironmentConfig {
    ...
    var enableFileLogging: Bool {
        boolValue(for: "ENABLE_FILE_LOGGING")
    }

    var enableRemoteLogging: Bool {
        boolValue(for: "ENABLE_CRASH_REPORTING")
    }
}
...
extension EnvironmentConfig {
    ...
    func configureLogger() {
        ...
        let minLevel = LogLevel.from(config.logLevel)
